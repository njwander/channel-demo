...
| 角色 | 职责描述 | 核心操作权限 |
| --- | --- | --- |
| 渠道负责人 | 一线执行者，负责渠道的引入与报备发起 | 发起入驻、录入报备、核对月度业绩、发起调账/解约 |
| 渠道负责人 | 一线执行者，负责渠道的引入与报备发起 | 发起入驻、录入报备、核对月度业绩、发起调账/解约 |
| 渠道TL | 业务管理者，把控准入评审与审批 | 入驻审批、报备评审（查重）、业绩调账审批、解约审批 |
| 渠道中台 | 规则监督者 | 日常数据巡检、规则配置维护 |
| 法务/CEO | 业绩把控者 | 月度结算单复核/终审 |
...
```mermaid
graph TD
    Start([开始]) --> Input[录入渠道信息和商务条款]
    
    %% 第一阶段：商务审批
    subgraph Stage1 [1. 商务准入与锁定]
        Input --> BizApproval[商务审批流<br>标准条款->TL / 非标->TL+CEO]
        BizApproval --> Approval{是否标准合作}
        Approval -- 标准 --> TLApproval[即心审批]
        Approval -- 非标 --> TLCEOApprove[即心->十布审批]
        TLApproval -- 审批通过 --> LockData[系统锁定商务数据]
        TLCEOApprove -- 审批通过 --> LockData[系统锁定商务数据]
    end

    Stage1 --> GenContract[系统自动生成合同模板]

    %% 第二阶段：合同分流
    subgraph Stage2 [2. 合同生成与法务]
        GenContract --> CheckType{选择合同模式?}
        
        %% 路径 A：标准 (跳过法务)
        CheckType -- 标准合同 (高频) --> UseStd[确定签约合同文件]
        
        %% 路径 B：非标 (法务介入)
        CheckType -- 非标合同 (低频) --> Modify[下载并修订后上传]
        Modify --> LegalApprove[渠道中台 -> 法务审批流]
        LegalApprove -- 通过 --> UseStd
    end

    %% 第三阶段：签署
    subgraph Stage3 [3. 签署与归档]
        UseStd --> Sign[发起签约<br>电子签 / 线下盖章]
        Sign --> Archive[系统归档]
    ...
    class Input,BizApproval,LockData,GenContract,UseStd,Modify,LegalApprove,Sign,Archive,Grant process;
...
> **调整后流程**
1. 影刀渠道负责人/影刀销售，在渠道系统中报备客户信息，报备信息可以指定推荐的销售，生成客户报备确认任务。
2. 如果是销售自己创建的客户报备，则确认任务由销售自己处理，如果是影刀渠道负责人创建的客户报备，则确认任务的负责人是影刀渠道负责人。
3. 报备负责人在CRM系统中查询客户是否存在，如果客户不存在则创建客户，关联渠道。
4. 如果客户已经存在，并且有在正常跟进中，那么报备失败。
5. 如果客户存在，但是不在正常跟进中，销售处理流程：申请认领客户到自己名下，关联渠道；渠道负责人处理流程：申请客户权限，把客户关联上渠道，再按规则分配给销售。
6. 如果客户关联上渠道，则客户报备进入保护期(30天)，如果客户在保护期内，销售有宣讲类型的跟进记录，则报备成功，超过保护期，报备失败。

500人以下的可以保护7天，500人以上的可以保护15天

每15天写一条跟进记录，没有写会掉库。

添加逻辑：客户从销售掉库的时候，需要移除渠道关系。

```mermaid
graph TD
    Start([开始]) --> Submit[渠道提交报备信息<br>生成报备确认任务]

    Submit --> CheckSpec{是否指定对接销售?}
    
    %% --- 左侧分支：销售处理 ---
        CheckSpec -- 指定销售 --> S_CheckCRM[销售查询 CRM]
        S_CheckCRM --> S_IsExist{客户是否存在?}
        
        %% 销售-客户不存在 (规则 3)
        S_IsExist -- 不存在 --> S_Create[销售创建客户]
        S_Create --> S_Link[销售关联渠道]
        
        %% 销售-客户存在 (规则 4, 5)
        S_IsExist -- 存在 --> S_CheckStatus{是否在正常跟进中?}
        S_CheckStatus -- 是 --> ManualFail[标记报备失败]
        S_CheckStatus -- 否 --> S_Apply{销售申请认领}
        
        S_Apply -- 申领成功 --> S_Link
        S_Apply -- 申领失败 --> ManualFail

    %% --- 右侧分支：渠道负责人处理 ---
        CheckSpec -- 未指定 --> M_CheckCRM[负责人查询 CRM]
        M_CheckCRM --> M_IsExist{客户是否存在?}
        
        %% 负责人-客户不存在 (规则 3)
        M_IsExist -- 不存在 --> M_Create[负责人创建客户]
        M_Create --> M_Link[负责人关联渠道]
        
        %% 负责人-客户存在 (规则 4, 5)
        M_IsExist -- 存在 --> M_CheckStatus{是否在正常跟进中?}
        M_CheckStatus -- 是 --> ManualFail
        
        %% 负责人-公海/沉寂客户处理 (规则 5)
        M_CheckStatus -- 否 --> M_LinkExisting[负责人关联客户到渠道]
        M_LinkExisting --> M_Assign[分配给销售]
    ...
    class Submit,S_CheckCRM,S_Create,S_Link,M_CheckCRM,M_Create,M_Link,M_LinkExisting,M_Assign,AutoFail,ManualFail process;
    class CheckTimeout,CheckSpec,S_IsExist,S_CheckStatus,S_Apply,M_IsExist,M_CheckStatus decision;
```
客户报备状态说明：
...
| 更新合同 | 商务审批后 & 签署前 | 下载当前合同，上传修订版附件，触发法务审批 |
| 发起签约 | 商务/法务审批通过 | 弹出签约配置弹窗 (电子签/线下盖章) |
...
| 点击【新建入驻申请】 | 跳转新建表单页 |
| 点击【下载合同】 | 在列表或详情页直接下载系统自动生成的合同草案 (PDF/Word) |
...
|  | 分佣规则 | 条件必填 | 分佣类型为"阶梯分佣"时必填，下拉选择已配置的规则（含系统默认规则）；**选择后需在下方以卡片形式展示该规则的具体阶梯明细** |
|  | 固定分佣比例 | 条件必填 | 分佣类型为"固定分佣"时必填，输入百分比 0-100 |
|  | 协议分佣说明 | 条件必填 | 分佣类型为"协议分佣"时必填，描述特殊分佣规则，最大 500 字符 |
...
> [!IMPORTANT]
> **商务审批路径**：
> - **标准路径**：分佣比例符合预设，审批流为：渠道负责人 -> 渠道负责人。
> - **非标路径**：分佣比例自定义，审批流跨级为：渠道负责人 -> 渠道负责人 -> CEO。
> 商务审批通过后，系统自动锁定相关商务条款。
...
│  列表区                                                                                      共 12 条  第 1/2 页     │
│  ┌───┬────────────┬──────┬────┬────┬────────┬────────┬────────┬────────┬────────┬────────┬──────────┬──────┬───┐  │
│  │序号│ 渠道名称    │ 状态  │比例│累计业绩 │推荐客户 │成交客户 │最近成交  │开始日期 │截止日期  │ 负责人   │操作│  │
│  ├───┼────────────┼──────┼────┼────────┼────────┼────────┼────────┼────────┼────────┼──────────┼──────┼───┤  │
│  │ 1 │ 成都XX科技..│合作中 │ 12%│ 120万  │   45   │   12   │01-10   │2025-01 │2026-01 │ 张三    │ ⋮ │  │
│  │ 2 │ 杭州YY信息..│即将到期│  8%│  85万  │   32   │    8   │01-05   │2025-03 │2026-03 │ 张三    │ ⋮ │  │
