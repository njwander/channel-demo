...
|  | 手机号 | 是 | 手机号格式校验 |
|  | 对接邮箱 | 是 | 邮箱格式校验，用于接收电子账单 |
|  | 担任职位 | 是 | 下拉选择职位 |
| 客户资源证明 | 初期提报客户盘点 | 否 | 意向客户工商名称（每行一个） |

**第二步：分佣配置信息录入**

此部分由影刀渠道负责人根据谈判结果录入分佣条款。
...
> **说明**：渠道订单核对是月度结算的核心环节，需要确认订单数据的准确性，业务端通常采用 offline 导出的方式获取渠道方的确认反馈，并导入系统。
...
│  │ 📊 核对进度概览                                                                              │   │
│  │ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐                         │   │
│  │ │  待核对订单   │ │  渠道已确认   │ │  确认通过    │ │  需人工处理   │                         │   │
│  │ │     15      │ │     28      │ │     25      │ │      3      │                         │   │
│  │ └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘                         │   │
│  └─────────────────────────────────────────────────────────────────────────────────────────────┘   │
├─────────────────────────────────────────────────────────────────────────────────────────────────────┤
│  [全部订单]  [待渠道确认]  [已确认通过]  [需人工处理]                                               │
├─────────────────────────────────────────────────────────────────────────────────────────────────────┤
│  查询区                                                                                             │
...
│  订单列表                                                              [批量确认]  [导入渠道确认]   │
│  ┌─────────────────────────────────────────────────────────────────────────────────────────────┐   │
│  │ □ 渠道名称      客户名称      合同编号    订单编号   订单产品   订单金额  回款日期  回款金额  渠道确认 操作│   │
│  ├─────────────────────────────────────────────────────────────────────────────────────────────┤   │
│  │ □ 成都飞逸计算机 四川XX有限公司 HT202512001 DD001    影刀标准版  39,800   12-05    39,800   ✅已确认  ⋮ │   │
│  │ □ 成都四方创享.. 重庆YY商贸.. HT202512002 DD002    影刀专业版  59,800   12-11    59,800   ⏳待确认  ⋮ │   │
│  │ □ 成都有为谷网络 贵州ZZ科技.. HT202512003 DD003    影刀企业版  59,800   12-19    59,800   ⏳待确认  ⋮ │   │
│  │ □ 西安胖柴信息.. 陕西AA企业.. HT202512004 DD004    影刀标准版  59,800   12-30    59,800   ✅已确认  ⋮ │   │
│  └─────────────────────────────────────────────────────────────────────────────────────────────┘   │
│                                                                          [< 上一页]  [下一页 >]     │
└─────────────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
| 状态 | 触发条件 | 后续操作 |
| --- | --- | --- |
| **待核对** | 订单刚导入，未确认 | 等待渠道确认结果导入 |
| **渠道已确认** | 渠道确认结果（无误）已导入 | 系统标记为确认通过 |
| **确认通过** | 确认结果已导入且无异议 | 自动通过，进入结算 |
| **需人工处理** | 渠道标记"有异议" | 渠道负责人人工处理 |
...
| 场景 | 处理方式 |
| --- | --- |
| 渠道确认"无误" | 系统自动将订单状态更新为"确认通过"，进入结算流程 |
| 渠道确认"有异议" | 状态变更为"需人工处理"，通知渠道负责人核实 |
...
```mermaid
graph TD
    A[每月初系统生成上月回款订单列表] --> B[系统发送核对通知给渠道负责人]
    B --> C2[渠道负责人导出核对清单]
    C2 --> D2[发送给渠道伙伴]
    D2 --> E2[渠道线下核对]
    E2 --> F2[渠道回传确认结果]
    F2 --> G2[渠道负责人修改订单或客户数据]
    G2 --> C2
    G2 --> L[进入月度结算单生成]
    L --> O[汇总生成月度结算单]
    O --> P[财务复核 → CEO终审]
