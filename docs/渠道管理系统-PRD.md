...
| 图表类型 | 展示数据 | 说明 |
| --- | --- | --- |
| 柱状图 | 各渠道周期累计业绩 | X轴：渠道名称，Y轴：业绩金额，按业绩降序排列 |
...
此部分由影刀渠道负责人根据谈判结果录入分佣条款。默认使用系统标准分佣阶梯。

| 分类 | 字段名称 | 必填 | 输入限制 |
| --- | --- | --- | --- |
| 分佣配置 | 分佣方案 | 是 | 默认：影刀标准分佣规则。点击“申请特殊分佣规则”可开启自定义模式。 |
|  | 分佣形式 | 条件必填 | 自定义模式下可选：阶梯分佣、固定分佣。 |
|  | 自定义配置 | 条件必填 | 阶梯模式下：直接编辑业绩区间与比例；固定模式下：录入统一分佣比例。 |
|  | 规则详情预览 | - | 标准模式下展示系统默认阶梯；自定义模式下展示配置后的明细。 |
...
- **Tab 1: 基础资料**
    ...
    - **联系人信息**：展示主要联系人信息（姓名、职位、电话、邮箱）。
    - **合规文件**：展示已入驻归档的合同文件，支持 PDF/Word 预览及下载。
- **Tab 2: 报备管理**
...
| 操作 | 可见条件 | 交互反馈 |
| --- | --- | --- |
| 解约 | 状态为"合作中" | 位于页面右上角【更多操作】下拉菜单中。点击后弹出解约弹窗，需填写解约原因、详细说明并上传凭证。确认后渠道状态变为"已解约"，相关报备权益锁定。 |
